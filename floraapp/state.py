...
AGENDA_STEP = 6
SUMMARY_STEP = 7
# El calendario se habilita a partir de septiembre de 2026.
MIN_DATE = date(2026, 9, 15)


def _calendar_floor() -> date:
    """Primer día seleccionable: a partir de septiembre de 2026."""
    return max(date.today(), MIN_DATE)

DEFAULTS = {
    "step": 1,              # Arranca directo en la confirmación Sí/No
    "answers": {},          # answers
    ...
    "current_exp": content.BASE_EXP,
    "prev_exp": content.BASE_EXP,
    "earned_exp": 0,
}
...
def init() -> None:
    ...
    if not st.session_state.step1_at:
...
def go(from_step: int, to_step: int) -> None:
    ...
    if to_step == 1:
        reset_escape()
    # Si avanza desde la elección del plan a la agenda, calculamos la EXP ganada
    if from_step == 2 and to_step == AGENDA_STEP:
        chosen_plan = st.session_state.answers.get(2)
        pts = content.PLAN_POINTS.get(chosen_plan, 0)
    ...
    st.session_state.cats = [
        {
            ...
            "delay": index * 0.2,
            "pick": random.randrange(len(content.FLOAT_CAT_SRCS)),
        }
