...
def step_confirm() -> None:
    ...
    with st.container(key="flora_card1"):
        _md(ui.step1_intro())
        with st.container(key="flora_yesno"):
            st.button(f"Sí {si_icon}", key="flora_si", on_click=state.say_yes)
            if attempts < 5:
                st.button("No", key="flora_no", on_click=state.escape_no)
...
def _calendar_panel() -> None:
    showing = st.session_state.temp_date
    with st.container(key="flora_cal"):
        # Fila con los extremos separados: el CSS la vuelve horizontal, igual que el
        # `justify-content: space-between` del original.
        with st.container(key="flora_cal_head"):
            st.button("←", key="flora_calnav_prev", on_click=state.shift_month, args=(-1,))
            _md(ui.month_label(showing))
            st.button("→", key="flora_calnav_next", on_click=state.shift_month, args=(1,))
        ...
        with st.container(key="flora_cal_grid"):
            for offset in range(0, len(cells), 7):
                ...
                for column, day in zip(columns, week):
                    with column:
                        ...
                        st.button(
                            str(day),
                            key=f"flora_day_{day}",
                            on_click=state.choose_day,
                            args=(day,),
                            disabled=date(showing.year, showing.month, day) < floor,
                        )

        st.button("Listo ✓", key="flora_cal_done", on_click=state.close_picker, width="stretch")


def _clock_panel() -> None:
    with st.container(key="flora_clock"):
        # Los tres controles van agrupados y centrados con 8px de separación.
        with st.container(key="flora_clock_row"):
            st.button("-", key="flora_clocknav_minus", on_click=state.shift_hour, args=(-1,))
            _md(ui.clock_face(st.session_state.temp_hour, st.session_state.temp_minute))
            st.button("+", key="flora_clocknav_plus", on_click=state.shift_hour, args=(1,))

        with st.container(key="flora_minutes"):
            for column, minute in zip(st.columns(4, gap=None), (0, 15, 30, 45)):
                with column:
                    st.button(
                        f":{minute:02d}",
                        key=f"flora_min_{minute}",
                        on_click=state.set_minute,
                        args=(minute,),
                        width="stretch",
                    )

        st.button("Listo ✓", key="flora_time_done", on_click=state.confirm_time, width="stretch")


def step_agenda() -> None:
    selected_date = st.session_state.selected_date
    selected_time = st.session_state.selected_time
    chosen_plan = st.session_state.answers.get(2)

    with st.container(key="flora_card2"):
        _md(ui.step2_intro(chosen_plan))
        ...
        if st.session_state.pick_error.get(state.AGENDA_STEP):
            _md(ui.error("// error: selecciona fecha y hora para coordinar 🗓️"))

        st.button(
            "[ CONFIRMAR FECHA → ]",
            key="flora_confirm_date",
...
def step_question(step: int) -> None:
    ...
    chosen = st.session_state.answers.get(step)
    _md(ui.progress(index))
    _md(ui.question_card(question, chosen))

    with st.container(key="flora_options"):
        for option_index, option in enumerate(question["options"]):
            st.button(
                option,
                key=f"flora_opt_{step}_{option_index}",
                on_click=state.pick,
                args=(step, option),
                width="stretch",
            )
    ...
    if st.session_state.pick_error.get(step):
        _md(ui.error("⚠ ¡Elegí una opción para coordinar! 🎮"))

    next_step = (
        content.QUESTIONS[index + 1]["id"]
        if index + 1 < len(content.QUESTIONS)
        else state.AGENDA_STEP
    )

    st.button(
        "[ CONFIRMAR OPCIÓN → ]",
        key=f"flora_next_{step}",
        on_click=state.go,
        args=(step, next_step),
        width="stretch",
    )
...
# Flujo: 1 confirmación (Sí/No) · 2 plan (+pts) · 6 agenda (EXP animada) · 7 resumen + final
ROUTES = {
    0: step_confirm,
    1: step_confirm,
    6: step_agenda,
