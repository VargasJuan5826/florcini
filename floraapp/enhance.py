...
def hover_to_click(token: str) -> None:
    ...
    (function () {{
      ...
      var onClick = function (e) {{
        ...
        if (hitYesNo(e.target, '.st-key-flora_no, .st-key-flora_si')) {{
      ...
      var onTouch = function (e) {{
        if (Date.now() > armedUntil) return;
        if (hitYesNo(e.target, '.st-key-flora_no, .st-key-flora_si') && e.cancelable) {{
          e.preventDefault();
        }}
      }};
      // Un solo par de listeners a la vez: el render anterior se limpia acá.
      if (doc.__floraGuard) {{
        doc.removeEventListener('click', doc.__floraGuard.c, true);
        doc.removeEventListener('touchstart', doc.__floraGuard.t, true);
      }}
      doc.__floraGuard = {{ c: onClick, t: onTouch }};
      ...
      var timer = setInterval(function () {{
        var no = doc.querySelector('.st-key-flora_no button');
        if (!no) return;
        if (Date.now() - start < 450) return;
        if (no.dataset.floraToken === TOKEN) {{ clearInterval(timer); return; }}
        no.dataset.floraToken = TOKEN;
